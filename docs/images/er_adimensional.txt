...
to setup
  ...
  make-turtles
  set a array:from-list n-values 100 [0]
  set b array:from-list n-values 100 [0]
  find-all-components
...
to make-turtles
  create-turtles num-nodes [ set size 2 ]
  ask turtles [
  ifelse (circular-initial-distribution? =  true)[
...
to go
  if ( (2 * count links ) >= ( (count turtles) * (count turtles - 1) ) or linkers = 0) [
    display
...
to go2
  if (sumy > (num-nodes * (num-nodes - 1)))[stop]
  set sumy (sumy + 1)
  if ( (2 * count links ) >= ( (count turtles) * (count turtles - 1) ) or linkers = 0) [
    display
    user-message "Network is fully connected. No more edges can be added."
...
to find-all-components
  ...
  [
    ...
    while [action]
    [
      if array:item a x = component-size [
        array:set b x ((array:item b x) + 1)
        set action false]
      if array:item a x = 0 [
        array:set b x 1
        array:set a x component-size
        set action false]
      set x x + 1
    ]
...
to add-edge
  ...
  ask node1 [
    ...
    [ if ((random-float 1) < p) [
      ...
      set linkers linkers - 1
      set linkers-in-use linkers-in-use + 1
      ]
    ]
  ]
end
...
to evaluate-network
    ...
    loop
    [
      if array:item a x = 0 [stop]
      set net-nodes net-nodes + (array:item a x * array:item b x)
      set net-numb net-numb + array:item b x
      set x x + 1
    ]
